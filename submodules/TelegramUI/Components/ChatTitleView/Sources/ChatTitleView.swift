                        ...
                        if peerView.peerId.isReplies {
                            ...
                            if let peer = peerView.peer {
                                if let customTitle {
                                    ...
                                    if SGSimpleSettings.shared.isStreamerActive && SGSimpleSettings.shared.streamerHideNames && peerView.peerId.namespace == Namespaces.Peer.CloudUser {
                                        segments = [.text(0, NSAttributedString(string: "User", font: titleFont, textColor: titleTheme.rootController.navigationBar.primaryTextColor))]
                                    } else if SGSimpleSettings.shared.isStreamerActive && SGSimpleSettings.shared.streamerHideGroupNames && (peerView.peerId.namespace == Namespaces.Peer.CloudGroup || peerView.peerId.namespace == Namespaces.Peer.CloudChannel) {
                                        segments = [.text(0, NSAttributedString(string: "Group", font: titleFont, textColor: titleTheme.rootController.navigationBar.primaryTextColor))]
                ...
                if let titleContent = self.titleContent {
                    switch titleContent {
                        case let .peer(peerView, customTitle, customSubtitle, onlineMemberCount, isScheduledMessages, _, customMessageCount, _, _):
                            if SGSimpleSettings.shared.isStreamerActive && SGSimpleSettings.shared.streamerHideStatuses && peerView.peerId.namespace == Namespaces.Peer.CloudUser {
