...
func contextMenuForChatPresentationInterfaceState(chatPresentationInterfaceState: ChatPresentationInterfaceState, context: AccountContext, messages: [EngineRawMessage], controllerInteraction: ChatControllerInteraction?, selectAll: Bool, interfaceInteraction: ChatPanelInterfaceInteraction?, readStats: MessageReadStats? = nil, messageNode: ChatMessageItemView? = nil) -> Signal<ContextController.Items, NoError> {
    ...
    |> map { data, updatingMessageMedia, infoSummaryData, appConfig, isMessageRead, messageViewsPrivacyTips, availableReactions, translationSettings, loggingSettings, notificationSoundList, accountPeer -> ContextController.Items in
        ...
        let isCopyProtected = (chatPresentationInterfaceState.copyProtectionEnabled || message.isCopyProtected()) && !SGSimpleSettings.shared.bypassCopyProtection
        ...
        if resourceAvailable, (!message.containsSecretMedia || SGSimpleSettings.shared.keepViewOnceMedia || SGSimpleSettings.shared.bypassCopyProtection) && (!isCopyProtected || SGSimpleSettings.shared.bypassCopyProtection) {
        ...
        ayuDateFormatter.locale = Locale(identifier: isRu ? "ru_RU" : "en_US")
        ayuDateFormatter.dateFormat = isRu ? "d MMMM, HH:mm" : "MMMM d, HH:mm"

        if SGAyugramStorage.shared.hasEditHistory(peerId: message.id.peerId.toInt64(), namespace: message.id.namespace, id: message.id.id) {
            let historyTitle = isRu ? "История изменений" : "Edit History"
            let editHistoryAction: ContextMenuItem = .action(ContextMenuActionItem(text: historyTitle, icon: { theme in
                return generateTintedImage(image: UIImage(bundleImageName: "Chat/Context Menu/Edit"), color: theme.actionSheet.primaryTextColor)
            }, action: { c, _ in
                c?.dismiss(completion: {
                    let history = SGAyugramStorage.shared.getEditHistory(peerId: message.id.peerId.toInt64(), namespace: message.id.namespace, id: message.id.id)
                    var sections: [String] = []
                    for item in history {
                        let dateStr = ayuDateFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(item.timestamp)))
                        sections.append("\(dateStr)\n\"\(item.text)\"")
                    }
                    let currentLabel = isRu ? "Текущая версия" : "Current version"
                    sections.append("\(currentLabel)\n\"\(message.text)\"")
                    let historyText = sections.joined(separator: "\n\n")
                    ...
                    controllerInteraction.presentController(textAlertController(
                        ...
                        text: historyText,
                        actions: [
                            TextAlertAction(type: .genericAction, title: isRu ? "Скопировать" : "Copy", action: {
        ...
        if SGAyugramStorage.shared.isDeleted(peerId: message.id.peerId.toInt64(), namespace: message.id.namespace, id: message.id.id) {
            let deletedMenuTitle = isRu ? "Сообщение удалено" : "Deleted Message"
            let deletedAction: ContextMenuItem = .action(ContextMenuActionItem(text: deletedMenuTitle, icon: { theme in
                ...
                c?.dismiss(completion: {
                    let entry = SGAyugramStorage.shared.getDeletedEntry(peerId: message.id.peerId.toInt64(), namespace: message.id.namespace, id: message.id.id)
                    let originalRaw = (entry?.text.isEmpty == false) ? entry!.text : message.text
                    let originalText = originalRaw.isEmpty ? (isRu ? "[Медиа / Пусто]" : "[Media / Empty]") : originalRaw
                    let timestamp = (entry?.timestamp ?? 0) != 0 ? entry!.timestamp : message.timestamp
                    let dateStr = ayuDateFormatter.string(from: Date(timeIntervalSince1970: TimeInterval(timestamp)))

                    let cardTitle = isRu ? "Сообщение удалено" : "Message Deleted"
                    let originalLabel = isRu ? "Оригинал:" : "Original:"
                    let cardText = "\(dateStr)\n\n\(originalLabel)\n\"\(originalText)\""
        ...
        if !sgActions.isEmpty {
            ...
            let swiftgramSubMenu: ContextMenuItem = .action(ContextMenuActionItem(text: "DoxGram", icon: { theme in
