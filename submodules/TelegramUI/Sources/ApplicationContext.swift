...
final class AuthorizedApplicationContext {
    ...
    init(sharedApplicationContext: SharedApplicationContext, mainWindow: Window1, context: AccountContextImpl, accountManager: AccountManager<TelegramAccountManagerTypes>, showContactsTab: Bool, showCallsTab: Bool, reinitializedNotificationSettings: @escaping () -> Void) {
        ...
        SGAyugramStorage.shared.setIsScreenCaptured(UIScreen.main.isCaptured)
        NotificationCenter.default.addObserver(forName: UIScreen.capturedDidChangeNotification, object: nil, queue: .main) { _ in
            SGAyugramStorage.shared.setIsScreenCaptured(UIScreen.main.isCaptured)
        }
        SGAyugramStorage.shared.cleanupExpired(retentionDays: SGSimpleSettings.shared.ayugramRetentionDays)
                ...
                if UIApplication.shared.applicationState == .active {
                    ...
                    if let appLockContext = strongSelf.context.sharedContext.appLockContext as? AppLockContextImpl {
                        ...
                        |> deliverOnMainQueue).start(next: { locked in
                            ...
                            if SGSimpleSettings.shared.isStreamerActive && SGSimpleSettings.shared.streamerHideNotifications {
                                return
                            }
