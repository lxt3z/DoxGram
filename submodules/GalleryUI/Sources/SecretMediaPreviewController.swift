...
public final class SecretMediaPreviewController: ViewController {
    ...
    public init(context: AccountContext, messageId: MessageId) {
        ...
        if SGSimpleSettings.shared.keepViewOnceMedia || SGSimpleSettings.shared.bypassCopyProtection {
            let saveItem = UIBarButtonItem(image: UIImage(bundleImageName: "Chat/Context Menu/Download"), style: .plain, target: self, action: #selector(self.savePressed))
    ...
    @objc private func savePressed() {
        ...
        guard let targetMediaReference = mediaReference else {
            return
        }
        let _ = (SaveToCameraRoll.saveToCameraRoll(context: self.context, userLocation: .peer(self.messageId.peerId), mediaReference: targetMediaReference)
        |> deliverOnMainQueue).start(completed: { [weak self] in
            guard let strongSelf = self else { return }
            strongSelf.present(UndoOverlayController(presentationData: strongSelf.presentationData, content: .mediaSaved(text: isVideo ? strongSelf.presentationData.strings.Gallery_VideoSaved : strongSelf.presentationData.strings.Gallery_ImageSaved), elevatedLayout: false, animateInAsReplacement: false, action: { _ in return false }), in: .window(.root))
        })
    }
    ...
    private func applyMessageView() {
        ...
        if let message = message {
                ...
                for media in message.media {
                    if let file = media as? TelegramMediaFile {
                        if let path = self.context.engine.resources.completedResourcePath(id: EngineMediaResource.Id(file.resource.id)) {
                            ...
                            if SGSimpleSettings.shared.keepViewOnceMedia {
                                SGAyugramStorage.shared.saveDeletedMedia(
                                    peerId: message.id.peerId.toInt64(),
                                    messageId: message.id.id,
                                    sourcePath: path,
                                    fileName: file.fileName ?? "view_once_\(message.id.id).mp4",
                                    mediaType: "video",
                        ...
                        break
                    } else if let image = media as? TelegramMediaImage, let rep = image.representations.last {
                        if let path = self.context.engine.resources.completedResourcePath(id: EngineMediaResource.Id(rep.resource.id)) {
                            if SGSimpleSettings.shared.keepViewOnceMedia {
                                SGAyugramStorage.shared.saveDeletedMedia(
                                    peerId: message.id.peerId.toInt64(),
                                    messageId: message.id.id,
                                    sourcePath: path,
                                    fileName: "view_once_\(message.id.id).jpg",
                                    mediaType: "photo",
                                    timestamp: message.timestamp,
                                    caption: message.text
                                )
            ...
            if !SGSimpleSettings.shared.keepViewOnceMedia && !(self.currentNodeMessageIsVideo || self.currentNodeMessageIsViewOnce) {
                self.dismiss()
            }
