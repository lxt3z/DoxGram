    ...
    present: @escaping (ViewController, Any?) -> Void) -> GalleryItem?
{
    ...
    if let image = media as? TelegramMediaImage {
        if let file = image.video {
            let captureProtected = (message.isCopyProtected() || message.containsSecretMedia || message.minAutoremoveOrClearTimeout == viewOnceTimeout || message.paidContent != nil || peerIsCopyProtected) && !SGSimpleSettings.shared.bypassCopyProtection && !SGSimpleSettings.shared.keepViewOnceMedia
