...
final class ChatItemGalleryFooterContentNode: GalleryFooterContentNode, ASScrollViewDelegate {
    ...
    func setMessage(_ message: EngineRawMessage, mediaSubject: GalleryMediaSubject? = nil, displayInfo: Bool = true, translateToLanguage: String? = nil, peerIsCopyProtected: Bool = false, displayPictureInPictureButton: Bool = false, settingsButtonState: SettingsButtonState? = nil, displayTextRecognitionButton: Bool = false, displayStickersButton: Bool = false, animated: Bool = false) {
        ...
        var canDelete: Bool
        var canShare = (!message.containsSecretMedia || SGSimpleSettings.shared.keepViewOnceMedia || SGSimpleSettings.shared.bypassCopyProtection) && !Namespaces.Message.allNonRegular.contains(message.id.namespace) && message.adAttribute == nil
        ...
        if (message.isCopyProtected() || peerIsCopyProtected || message.paidContent != nil) && !SGSimpleSettings.shared.bypassCopyProtection {
            canShare = false
            canEdit = false
        }
        
        if message.containsSecretMedia && !SGSimpleSettings.shared.keepViewOnceMedia && !SGSimpleSettings.shared.bypassCopyProtection {
            canDelete = false
        }
