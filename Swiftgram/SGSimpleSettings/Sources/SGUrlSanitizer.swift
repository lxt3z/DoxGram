...
public final class SGUrlSanitizer {
    ...
    private static let ipLoggerDomains: Set<String> = [
        ...
        ip-score.com, whoer.net, speedtest.net/ip, iplocation.net
    ]
 ...
 let isTelegram = telegramHosts.contains { host == || host.hasSuffix(. + ) }
 if !isTelegram {
 return SuspiciousCheckResult(
 isSuspicious: true,
 isIpGrabber: false,
 ...
 if let target = components.queryItems?.first(where: { .name == q || .name == url })?.value {
 return target
 }
 }
 ...
 if let target = components.queryItems?.first(where: { .name == to })?.value {
 return target
 }
 }
 ...
 if let target = components.queryItems?.first(where: { .name == q })?.value {
 return target
 }
 }
 ...
 if let target = components.queryItems?.first(where: { .name == u })?.value {
 return target
 }
 }
